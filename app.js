...
const officialOcean = { source: '부산생활지도 ArcGIS REST / 기상청 해양관측', endpoint: 'https://lifemap.busan.go.kr/arcgis/rest?f=json', apiKey: '' };
const busanOceanPoints = [
  {place:'해운대', lat:35.1587, lng:129.1603, risk:'high', detail:'보름달물해파리 다수 관측 · 수온 25.4°C'},
  {place:'광안리', lat:35.1532, lng:129.1187, risk:'low', detail:'해파리 단건 관측 · 수온 25.1°C'},
  {place:'송정', lat:35.1784, lng:129.1995, risk:'clear', detail:'특이사항 없음 · 파고 0.4m'},
  {place:'다대포', lat:35.0460, lng:128.9657, risk:'low', detail:'보름달물해파리 소량 · 수온 24.8°C'},
  {place:'송도', lat:35.0752, lng:129.0168, risk:'clear', detail:'정기 관찰 구간 · 이상 징후 없음'},
  {place:'일광', lat:35.2648, lng:129.2512, risk:'clear', detail:'동부 해안 관측 구간 · 현장 확인 필요'}
];
const beachReports = [
...
async function refreshOceanStatus(){ const status=qs('#apiStatus'), message=qs('#apiMessage'), dot=document.querySelector('.api-dot'); if(!status) return; status.textContent='부산생활지도 API 확인 중'; message.textContent='공식 ArcGIS REST 레이어 목록을 확인하고 있습니다…'; dot.classList.remove('connected'); try { const response=await fetch(officialOcean.endpoint,{mode:'cors',cache:'no-store'}); const contentType=response.headers.get('content-type')||''; const body=await response.text(); if(contentType.includes('json') && body.trim().startsWith('{')){ status.textContent='부산생활지도 공식 API 연결됨'; message.textContent='ArcGIS REST 레이어 응답 확인 · 부산 6개 해안 관측 구간 표시'; dot.classList.add('connected'); } else { status.textContent='부산생활지도 API 점검 중'; message.textContent='공식 REST 주소는 확인되지만 현재 레이어 JSON 대신 점검 안내가 반환되어 좌표 기반 관측 구간을 표시합니다.'; } } catch(error){ status.textContent='부산생활지도 API 접근 확인 필요'; message.textContent='공개 정적 페이지에서 REST 응답을 읽지 못해 좌표 기반 관측 구간을 표시합니다.'; } }
renderBusanDetailMap(); refreshOceanStatus();

// 전국 해안 관측 지도: Google Maps JavaScript API가 설정되면 실제 지도, 미설정 시 전국 해안 대체 화면을 표시합니다.
...
const googleMapConfig = {apiKey:window.JELLYWATCH_GOOGLE_MAPS_API_KEY || ''};
...
function renderNationalFallback(map){if(!map)return;map.className='map-canvas national-map national-fallback';map.innerHTML=`<div class="grid-lines"></div><div class="national-land"><span class="land-title">대한민국</span></div><div class="fallback-caption"><strong>전국 해안 관측 범위</strong><span>서해 · 남해 · 동해 · 제주</span></div>${nationalReports.map((r,i)=>`<button class="national-pin ${r.risk}" style="--x:${nationalPinPositions[i][0]}%;--y:${nationalPinPositions[i][1]}%" data-place="${r.place}" aria-label="${r.place} ${r.level}">${i+1}</button>`).join('')}<div class="busan-inset"><div class="busan-inset-head"><strong>부산 연안 확대</strong><span>공식 API 기준 구간</span></div><div class="busan-inset-coast"></div>${busanOceanPoints.map((p,i)=>`<button class="busan-inset-pin ${p.risk}" style="--x:${[72,61,80,25,45,91][i]}%;--y:${[22,43,9,80,67,28][i]}%" data-place="부산 ${p.place}" aria-label="부산 ${p.place}">${i+1}</button>`).join('')}<div class="busan-inset-label">동부산</div><div class="busan-inset-label west">서부산</div></div><div class="map-scale">전국 해안 모니터링 <span>부산 6개 구간 확대</span></div>`;bindNationalInteractions();}
function selectNationalReport(place){document.querySelectorAll('.report').forEach(r=>r.classList.toggle('active',r.dataset.place===place));document.querySelector(`.report[data-place="${place}"]`)?.scrollIntoView({behavior:'smooth',block:'nearest'});const item=nationalReports.find(r=>r.place===place);if(window.googleNationalMap&&item){window.googleNationalMap.panTo({lat:item.lat,lng:item.lng});window.googleNationalMap.setZoom(10);}}
function selectBusanPoint(place){const item=busanOceanPoints.find(p=>`부산 ${p.place}`===place);if(window.googleNationalMap&&item){window.googleNationalMap.panTo({lat:item.lat,lng:item.lng});window.googleNationalMap.setZoom(12);}document.querySelectorAll('.report').forEach(r=>r.classList.toggle('active',r.dataset.place===place));}
function bindNationalInteractions(){document.querySelectorAll('.national-pin').forEach(pin=>pin.addEventListener('click',()=>selectNationalReport(pin.dataset.place)));document.querySelectorAll('.busan-inset-pin').forEach(pin=>pin.addEventListener('click',()=>selectBusanPoint(pin.dataset.place)));document.querySelectorAll('.report').forEach(report=>report.addEventListener('click',()=>selectNationalReport(report.dataset.place)));}
function renderGoogleMap(){const map=qs('.map-canvas');if(!map||!window.google?.maps)return;map.className='map-canvas national-map';map.innerHTML='<div id="googleMap" class="google-map-host"></div>';window.googleNationalMap=new google.maps.Map(document.getElementById('googleMap'),{center:{lat:36.35,lng:127.85},zoom:7,minZoom:6,maxZoom:14,streetViewControl:false,fullscreenControl:true,mapTypeControl:true,gestureHandling:'cooperative'});nationalReports.forEach((r,i)=>{const marker=new google.maps.Marker({position:{lat:r.lat,lng:r.lng},map:window.googleNationalMap,title:r.place,label:{text:String(i+1),color:'#fff',fontWeight:'700'},icon:{path:google.maps.SymbolPath.CIRCLE,scale:14,fillColor:r.risk==='high'?'#f06f58':r.risk==='low'?'#3a9e9a':'#6b8da8',fillOpacity:1,strokeColor:'#fff',strokeWeight:2}});marker.addListener('click',()=>selectNationalReport(r.place));});busanOceanPoints.forEach((p,i)=>{const marker=new google.maps.Marker({position:{lat:p.lat,lng:p.lng},map:window.googleNationalMap,title:`부산 ${p.place} · ${p.detail}`,label:{text:`B${i+1}`,color:'#fff',fontWeight:'700'},icon:{path:google.maps.SymbolPath.CIRCLE,scale:11,fillColor:'#ff744d',fillOpacity:.95,strokeColor:'#fff',strokeWeight:2}});marker.addListener('click',()=>selectNationalReport(`부산 ${p.place}`));});}
function updateNationalStatus(connected){const status=qs('#apiStatus'),message=qs('#apiMessage');if(!status)return;if(connected){status.textContent='Google 지도 API 연결됨';message.textContent='전국 해안 12개 관측 거점 · 마커를 눌러 지역별 제보 확인';}else{status.textContent='Google 지도 API 키 확인 필요';message.textContent='API 키가 없거나 인증에 실패해 전국 해안 대체 지도를 표시합니다.';}}
function loadGoogleMap(){const map=qs('.map-canvas');if(!googleMapConfig.apiKey){renderNationalFallback(map);return;}const script=document.createElement('script');script.src=`https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(googleMapConfig.apiKey)}&loading=async&callback=jellyWatchGoogleMapReady&language=ko&region=KR`;script.async=true;script.defer=true;window.jellyWatchGoogleMapReady=()=>{renderGoogleMap();updateNationalStatus(true);};script.onerror=()=>{renderNationalFallback(map);updateNationalStatus(false);};document.head.appendChild(script);}
function renderNationalMap(){const mapLayout=qs('.map-layout'),map=qs('.map-canvas'),list=qs('.report-list');if(!mapLayout||!map||!list)return;mapLayout.classList.add('national-layout');list.innerHTML=nationalReportMarkup();const bar=qs('.official-data-bar');if(bar){bar.querySelector('.source-link').href='https://developers.google.com/maps/documentation/javascript';bar.querySelector('.source-link').textContent='Google Maps API 문서 ↗';}bindNationalInteractions();loadGoogleMap();updateNationalStatus(false);}
renderNationalMap();
