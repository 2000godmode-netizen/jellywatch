...
// 부산 연안만 표시: Google Maps JavaScript API가 설정되면 실제 지도, 미설정 시 부산 상세 대체 지도를 표시합니다.
const nationalReports = busanOceanPoints.map((p) => ({place:`부산 ${p.place}`,region:'부산 연안',lat:p.lat,lng:p.lng,level:p.risk==='high'?'주의':p.risk==='low'?'관찰':'양호',risk:p.risk,species:'해파리 관측',note:p.detail}));
const nationalPinPositions = [[72,22],[61,43],[80,9],[25,80],[45,67],[91,28]];
const googleMapConfig = {apiKey:window.JELLYWATCH_GOOGLE_MAPS_API_KEY || ''};
function nationalReportMarkup(){return `<div class="list-heading"><span>부산 연안 관측 제보</span><span class="mono">${nationalReports.length} COASTS</span></div>`+nationalReports.map((r,i)=>`<article class="report${i===0?' active':''}" data-place="${r.place}" data-level="${r.level}"><div class="report-top"><span class="status ${r.risk}">${r.level}</span><span class="mono">${r.lat.toFixed(4)}°N</span></div><h3>${r.place}</h3><p>${r.species} · ${r.note}</p><div class="report-foot"><span>${r.region}</span><button class="more">상세 보기 ↗</button></div></article>`).join('');}
function renderNationalFallback(map){if(!map)return;map.className='map-canvas busan-only-map national-fallback';map.innerHTML=`<div class="grid-lines"></div><div class="fallback-caption"><strong>부산 연안 관측 지도</strong><span>해운대 · 광안리 · 송정 · 다대포 · 송도 · 일광</span></div><div class="busan-coast"></div>${nationalReports.map((r,i)=>`<button class="national-pin ${r.risk}" style="--x:${nationalPinPositions[i][0]}%;--y:${nationalPinPositions[i][1]}%" data-place="${r.place}" aria-label="${r.place} ${r.level}">${i+1}</button>`).join('')}<div class="map-label busan-map-label">부산 연안</div><div class="map-scale">부산생활지도 기준 <span>6개 관측 구간</span></div>`;bindNationalInteractions();}
function selectNationalReport(place){document.querySelectorAll('.report').forEach(r=>r.classList.toggle('active',r.dataset.place===place));document.querySelector(`.report[data-place="${place}"]`)?.scrollIntoView({behavior:'smooth',block:'nearest'});const item=nationalReports.find(r=>r.place===place);if(window.googleNationalMap&&item){window.googleNationalMap.panTo({lat:item.lat,lng:item.lng});window.googleNationalMap.setZoom(13);}}
...
function renderGoogleMap(){const map=qs('.map-canvas');if(!map||!window.google?.maps)return;map.className='map-canvas busan-only-map';map.innerHTML='<div id="googleMap" class="google-map-host"></div>';window.googleNationalMap=new google.maps.Map(document.getElementById('googleMap'),{center:{lat:35.16,lng:129.14},zoom:11,minZoom:10,maxZoom:16,streetViewControl:false,fullscreenControl:true,mapTypeControl:true,gestureHandling:'cooperative'});nationalReports.forEach((r,i)=>{const marker=new google.maps.Marker({position:{lat:r.lat,lng:r.lng},map:window.googleNationalMap,title:`${r.place} · ${r.note}`,label:{text:String(i+1),color:'#fff',fontWeight:'700'},icon:{path:google.maps.SymbolPath.CIRCLE,scale:14,fillColor:r.risk==='high'?'#f06f58':r.risk==='low'?'#3a9e9a':'#6b8da8',fillOpacity:1,strokeColor:'#fff',strokeWeight:2}});marker.addListener('click',()=>selectNationalReport(r.place));});}
function updateNationalStatus(connected){const status=qs('#apiStatus'),message=qs('#apiMessage');if(!status)return;if(connected){status.textContent='Google 지도 API 연결됨';message.textContent='부산 연안 6개 관측 구간 · 마커를 눌러 지역별 제보 확인';}else{status.textContent='Google 지도 API 키 확인 필요';message.textContent='API 키가 없거나 인증에 실패해 부산 연안 대체 지도를 표시합니다.';}}
...
function renderNationalMap(){const mapLayout=qs('.map-layout'),map=qs('.map-canvas'),list=qs('.report-list');if(!mapLayout||!map||!list)return;mapLayout.classList.add('busan-only-layout');list.innerHTML=nationalReportMarkup();const bar=qs('.official-data-bar');if(bar){bar.querySelector('.source-link').href='https://lifemap.busan.go.kr/arcgis/rest?f=json';bar.querySelector('.source-link').textContent='부산생활지도 ArcGIS REST ↗';}bindNationalInteractions();loadGoogleMap();}
renderNationalMap();
