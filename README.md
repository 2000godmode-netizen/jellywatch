...
부산 구간은 [부산생활지도 ArcGIS REST](https://lifemap.busan.go.kr/arcgis/rest?f=json) 상태를 확인하면서 해운대·광안리·송정·다대포·송도·일광 6개 관측 구간을 확대 표시합니다. 부산생활지도 서버가 점검 중이거나 레이어 응답을 읽을 수 없을 때는 공식 API 주소와 상태를 표시하고, 좌표 기반 안전한 대체 지도를 사용합니다.
